...
class LicenseBoxAPI{
	...
	public function __construct(){ 
		$this->product_id = '55248C11';
		$this->api_url = 'https://licencefly.com/';
		$this->api_key = '4B0E4EC28B77669-thebrbr';
	...
	public function verify_license($time_based_check = false, $license = false, $client = false){
		...
		if($time_based_check && $this->verification_period > 0){
			...
			$today = date('d-m-Y');
			if(empty($_SESSION["aa8b6f44c1f2fc3"])){
				$_SESSION["aa8b6f44c1f2fc3"] = '00-00-0000';
			}
			if($type == 1){
				$type_text = '1 day';
			}elseif($type == 3){
			...
			if(strtotime($today) >= strtotime($_SESSION["aa8b6f44c1f2fc3"])){
				$get_data = $this->call_api(
					'POST',
